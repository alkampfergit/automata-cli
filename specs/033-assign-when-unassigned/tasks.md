...
- [ ] **T001** Add `assignees: string[]` to `PrSurface` in `src/github/ghWorkService.ts`,
...
- [ ] **T002** Add `assignPrToAgent(prNumber, agentUser)` to
...
- [ ] **T003** [P] Add `assignees` to `getCurrentBranchPr()`'s `--json` field list and
  return type in `src/config/githubService.ts`, normalising an absent field to `[]`.
- [ ] **T004** Extend `tests/unit/ghWorkService.test.ts`: `getPrSurface` exposes
...
- [ ] **T005** In `src/github/workDetection.ts`, change `needsAssignment` to
  ...
  all".
- [ ] **T006** Rewrite the two existing assignment tests in
...
- [ ] **T011** In `repairIssueLink()` (`src/commands/doWork.ts`), claim the pull request
...
- [ ] **T012** Extend `tests/unit/doWork.cmd.test.ts`: a build turn on an unassigned
...
- [ ] **T013** Update `describePlannedRun()`'s `Assignment` line in
...
- [ ] **T014** Add `prNeedsAssignment` to `toPlanJson()` in `src/commands/doWork.ts`.
- [ ] **T015** Extend `tests/unit/doWork.cmd.test.ts` for the dry-run text in each
...
- [ ] **T016** In `linkPrToIssue()` (`src/commands/getReady.ts`), claim the pull
...
- [ ] **T017** Add a test covering the `implement-next` pull-request claim: claimed
  when unassigned, skipped when assigned, warning on failure.

## Phase 6 — Documentation and gates

- [ ] **T018** Update `docs/do-work.md`: replace the current issue-claim description
...
- [ ] **T019** Document the `implement-next` pull-request claim in the relevant
  command-group page.
- [ ] **T020** Typecheck the touched test files explicitly
  (`npx tsc --noEmit --strict …`), since `tsconfig.json` covers only `src` and vitest
  does not typecheck.
- [ ] **T021** Run `npm test` and the real lint gate (`rtk proxy npm run lint` /
