...
- [ ] **T001** Add `"pr-orphan"` to `TurnKind` and `prOrphan?: string` to `DoWorkPrompts`; export
...
- [ ] **T002 [P]** Test in `tests/unit/configStore.test.ts`: `prOrphan: "pr-orphan.md"` resolves from
  `.automata/`, and a path escaping `.automata/` throws. (FR-006)

## Phase 2 — Orphan discovery

- [ ] **T003** Extend `LINK_MAP_QUERY` with `labels(first:50){nodes{name}}` and
...
- [ ] **T004** Test in `tests/unit/ghWorkService.test.ts`: a pull request closing nothing is an
...
- [ ] **T005** In `src/github/workDetection.ts`: make `WorkItem.issue` `GitHubIssue | null`; add
  ...
  (FR-005, FR-014)
- [ ] **T006** Add `OrphanPrState` and `decideOrphanPrWork(state, participants, policy)`: skip a
...
- [ ] **T007** Tests in `tests/unit/workDetection.test.ts` for the orphan decision table: a new
...
- [ ] **T008** In `src/github/workPrompt.ts`: emit the issue identity, the issue conversation and the
...
- [ ] **T009 [P]** Tests in `tests/unit/workPrompt.test.ts`: a `pr-orphan` prompt names the pull
...
- [ ] **T010** `src/commands/doWork.ts` plumbing: add `--pr <number>` and `onlyPr` to `Settings`;
  add the `pr-orphan` prompt to `Settings.prompts`; accept `prOrphan` in
  `validateSettingContainer(section["prompts"], …)`. (FR-006, FR-008)
- [ ] **T011** Orphan discovery in the command: `prMatchesFilter(candidate, settings)` mirroring
...
- [ ] **T012** The second pass: run it after the issue pass; suppress the issue pass when `--pr` is
...
- [ ] **T013** Per-item handling for `pr-orphan`: refresh against a fresh link map and skip on
...
- [ ] **T014** Reporting: `ItemReport.issue` nullable plus a new `pr` field; an `itemLabel()` helper
...
- [ ] **T015** Tests in `tests/unit/doWork.cmd.test.ts`: a matching orphan pull request with an
...
- [ ] **T016** `src/commands/config.ts`: add `"pr-orphan"` to `VALID_TURN_KINDS` and replace the
  if/else in `do-work-prompt` with an exhaustive switch over the three kinds. (FR-006)
- [ ] **T017 [P]** Test in `tests/unit/config.cmd.test.ts`:
...
- [ ] **T018** `src/config/ConfigWizard.tsx`: append `"Do Work — Orphan PR"` to
...
- [ ] **T019 [P]** Test in `tests/unit/ConfigWizard.test.tsx`: navigating Prompts → Do Work — Orphan
  PR and submitting writes the prompt file and `doWork.prompts.prOrphan`. (FR-006)

## Phase 7 — Documentation

- [ ] **T020** `docs/do-work.md`: `--pr` in the options table, the `pr-orphan` row in Turn kinds, the
...
- [ ] **T021 [P]** `docs/config.md`: `prompts.prOrphan` in the `doWork` table and in the example.
- [ ] **T022 [P]** `docs/wiki/Detection-Rules.md`: the orphan pull-request rule beside the issue and
  pull-request rules.
- [ ] **T023** Review `README.md` per the constitution: update only if the command-group table, the
  quick start, installation or dev setup actually changed.

## Phase 8 — Gate

- [ ] **T024** `npm test && npm run lint` (read the real lint gate with `npx eslint src/` per project
