...
- [ ] T001 Add the git wrappers `do-work` hygiene needs to `src/git/gitService.ts`, each returning
...
- [ ] T002 Add the two `gh` wrappers to `src/github/ghWorkService.ts`, with private `Raw…`
...
- [ ] T003 [P] Unit-test every wrapper from T001 in `tests/unit/gitService.hygiene.test.ts` by
...
- [ ] T004 [P] Unit-test both wrappers from T002 in `tests/unit/ghWorkService.test.ts`: argv,
...
- [ ] T005 [US1] Create `src/git/repoHygiene.ts` with the types from `data-model.md`
...
- [ ] T006 [US1] Implement `rescueUncommittedChanges`: trigger on
...
- [ ] T007 [US1] Handle detached HEAD as the base-branch case, and never commit to or push
  `baseBranch`
- [ ] T008 [US1] Honour `dryRun`: report `would-rescue` and issue no mutating call
- [ ] T009 [US1] Call `runRepoHygiene` from `runTick` in `src/commands/doWork.ts` — inside the run
  lock, before `discoverIssues` — and thread the report through to reporting
- [ ] T010 [US1] Report the pre-flight: a `Pre-flight:` block on stderr as it runs, a section in
  `summarize()`, a `preflight` object in the `--json` payload, and inclusion in `reportDryRun`
- [ ] T011 [US1] Fold `report.degraded` into the tick's exit code: force 2 where the tick would
  otherwise return 0, leaving exit 1 untouched
- [ ] T012 [P] [US1] Unit-test the rescue in `tests/unit/repoHygiene.test.ts`: clean tree does
...
- [ ] T013 [P] [US1] Extend `tests/unit/doWork.cmd.test.ts`: the pre-flight runs exactly once per
...
- [ ] T014 [US2] Add the base step to `runRepoHygiene`: `checkoutBranch(baseBranch)` then
  `pullFastForwardOnly()`, after the rescue, populating `report.base`
- [ ] T015 [P] [US2] Unit-test in `tests/unit/repoHygiene.test.ts`: it runs with zero actionable
...
- [ ] T016 [US3] Add the prune step to `runRepoHygiene`, after the base step: list local and
...
- [ ] T017 [US3] Per candidate: `listPullRequestsForHead`; keep on an `OPEN` pull request; keep
  ...
  `lookup-failed`
- [ ] T018 [US3] Honour `dryRun` with `would-delete` / `would-rescue`
- [ ] T019 [P] [US3] Unit-test the full decision table in `tests/unit/repoHygiene.test.ts`, one
...
- [ ] T020 [US4] Verify and complete `--dry-run` end to end: assert in
...
- [ ] T021 [US4] Assert in `tests/unit/doWork.cmd.test.ts` that `--dry-run` reports the pre-flight
...
- [ ] T022 Document the pre-flight in `docs/do-work.md`: a new section covering all three steps
...
- [ ] T023 Confirm `README.md` needs no change — no installation, quick-start, command-group or
  dev-setup change (per the documentation convention in `AGENTS.md`)
- [ ] T024 Run `npm test && npm run lint` (reading the real lint gate with `npx eslint src/`, per
