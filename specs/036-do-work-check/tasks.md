...
- [X] **T001** [P] `src/run/runLock.ts`: add the `LockStatus` union and
  ...
  (R1, FR-002, FR-005)
- [X] **T002** [P] `tests/unit/runLock.test.ts`: cases for `free`, `held` (live, in
...
- [X] **T003** [P] `src/run/operationLog.ts`: add `ExecutionTick`, `WorkRecord`,
...
- [X] **T004** [P] `tests/unit/operationLog.read.test.ts`: round-trip tests —
...
- [X] **T005** [P] `src/git/repoStatus.ts`: `RepoStatus` and
...
- [X] **T006** [P] `tests/unit/repoStatus.test.ts`: clean/level, dirty with paths,
...
- [X] **T007** `src/run/checkReport.ts`: the `CheckReport` / `CheckSection` / `Problem`
...
- [X] **T008** `tests/unit/checkReport.test.ts`: the cadence heuristic (median, the
...
- [X] **T009** `src/commands/doWork.ts`: split `resolveSettings` into
...
- [X] **T010** `src/commands/doWork.ts`: add the `--check` and `--no-fetch` options with
...
- [X] **T011** `src/commands/doWork.ts`: `runCheck(options)` — assemble the four local
...
- [X] **T012** `tests/unit/doWorkCheck.cmd.test.ts`: the command-level tests —
...
- [X] **T013** [P] `docs/do-work.md`: a "Checking the loop's health" section — what each
...
- [X] **T014** [P] `CHANGELOG.md`: one bullet under `## [Unreleased]` — `do-work` gained
  `--check` and `--no-fetch`.
- [X] **T015** [P] `AGENTS.md`: add the feature to `## Recent Changes`.

## Phase E — Gate

- [X] **T016** `npm test && npm run lint` green; `npx prettier --check` on every new file.
- [X] **T017** Smoke-run `node dist/index.js do-work --check` and
